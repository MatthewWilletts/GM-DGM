...
"""
Implementation of Gaussian Mixture DGMs from Willetts et al. (2018):
...
class gm_dgm(model):
    ...
    def __init__(self, n_x, n_y, n_z=2, x_dist='Gaussian', mc_samples=1,
                 ...
                 learning_paradigm='supervised', name='gm_dgm', prior=None,
        ...
        super(gm_dgm, self).__init__(n_x, n_y, n_z, x_dist, mc_samples, l2_reg,
    ...
    def build_model(self):
        ...
        ## Generative Networks:
        # Make p(z|y) network - again two, one for mean one for log variance
        self.p_z_y_mean = Sequential()
        self.p_z_y_mean.add(Dense(self.n_z,
                                  input_dim=self.n_y,
                                  kernel_initializer=normal_initializer,
                                  bias_initializer='zeros'))

        self.p_z_y_log_var = Sequential()
        self.p_z_y_log_var.add(Dense(self.n_z,
                                     input_dim=self.n_y,
                                     kernel_initializer=glorot_initializer,
                                     bias_initializer='zeros'))
        # Make p(x|z) network - if Gaussain: two, one for mean one for log var
        #                     - if Bernoulii: just one for prob for each entry

        if self.x_dist == 'Gaussian':
            self.p_x_z_model = Sequential()
            self.p_x_z_model.add(Dense(self.intermediate_dim,
                                       kernel_initializer=glorot_initializer,
                                       bias_initializer=normal_initializer,
                                       input_dim=self.n_z))
            self.p_x_z_model.add(Activation('relu'))
            self.p_x_z_model.add(Dense(self.intermediate_dim,
                                       kernel_initializer=glorot_initializer,
                                       bias_initializer=normal_initializer))
            self.p_x_z_model.add(Activation('relu'))
            ...
            self.p_x_z_mean.add(self.p_x_z_model)
            self.p_x_z_mean.add(Dense(self.n_x,
                                      kernel_initializer=normal_initializer,
                                      bias_initializer=normal_initializer))

            self.p_x_z_log_var = Sequential()
            self.p_x_z_log_var.add(self.p_x_z_model)
            self.p_x_z_log_var.add(Dense(self.n_x,
                                         kernel_initializer=normal_initializer,
                                         bias_initializer=normal_initializer))
            #
    ...
    def _printing_feed_dict(self, Data, x_l, x_u, y, eval_samps, binarize):
        fd = super(gm_dgm, self)._printing_feed_dict(Data, x_l, x_u, y,
                                                     eval_samps, binarize)
